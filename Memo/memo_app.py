...
count=2
for record in records:
    current_rec = str(record[3]) + " " + str(record[0]) + " " + str(record[1]) + " " + str(record[2]).split()[0] + "\n"
    current_id = record[3]
    print(current_rec[0])
    query_btn = Button(root, text=current_rec, height=2, width=100, command=partial(open, current_id))
